...
class DictionnaireAdjacenceOrientePondere(object):
    ...
    def ajouter_arc_bidirectionel(self, u, v, poids):
        """Ajoute un arc bidirectionnel du sommmet u vers le sommet v et vice-versa, en créant les sommets
        manquants le cas échéant."""
        self.ajouter_arc(u, v, poids)
        self.ajouter_arc(v, u, poids)

    def ajouter_arcs_bidirectionel(self, iterable):
        """Ajoute tous les arcs bidirectionnels de l'itérable donné au graphe. N'importe
        quel type d'itérable est acceptable, mais il faut qu'il ne contienne
        que des couples d'éléments (quel que soit le type du couple)."""
        for u, v, poids in iterable:
            self.ajouter_arc_bidirectionel(u, v, poids)
